			    ...
			    u64 *tokenp)
{
	...
	ret = liveupdate_get_file_outgoing(s, idev->ictx->file, tokenp);
	if (ret)
		goto out;
	...
	ret = iommu_preserve_device(hwpt_paging->common.domain,
				    ...
				    *tokenp);
	if (ret)
		liveupdate_put_file_outgoing(s, idev->ictx->file, *tokenp);
			       ...
			       u64 token)
{
	...
	liveupdate_put_file_outgoing(s, idev->ictx->file, token);
out:
	mutex_unlock(&igroup->lock);
}
