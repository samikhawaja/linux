						   ...
						   struct iommu_hw_ser *ser)
{
	...
	/*
	 * Issue global invalidations to remove references to unpreserved tables
	 * and entries.
	 */
					 ...
					 struct iommu_hw_ser *ser)
{
	...
	for (i = 0; i < ROOT_ENTRY_NR; i++) {
		ret = preserve_context_table(iommu, ser, i, 0);
		if (ret)
			goto error;
		...
		ret = preserve_context_table(iommu, ser, i, 0x80);
		if (ret)
			goto error;
	}
			 ...
			 struct iommu_hw_ser *ser)
{
	...
	/* Root is already preserved if updating */
