...
void intel_iommu_unpreserve_device(struct device *dev, struct device_ser *device_ser)
{
	...
	ret = pasid_walk_table(pasid_table->table, pasid_table->max_pasid,
			       PASID_WALK_UNPRESERVE, -1);
	WARN_ON(ret);
}
...
void *intel_pasid_try_restore_table(struct device *dev, u64 max_pasid)
{
	...
	if (WARN_ON_ONCE(ser->intel.max_pasid != max_pasid)) {
		pasid_walk_table(phys_to_virt(ser->intel.pasid_table),
				 ser->intel.max_pasid, PASID_WALK_FREE, -1);
		return NULL;
	}
