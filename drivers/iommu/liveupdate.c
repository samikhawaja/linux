...
/**
 * alloc_object_ser() - Allocate the next available slot in a linked list of arrays
 * @curr_array_ptr: A double pointer to the "current" array tracking pointer
 *                  (e.g., &flb->curr_domain_array). This allows the allocator
 *                  to update the current pointer when a new page is chained.
 * @max_objs:       The maximum number of objects that fit in one array page.
 *
 * This generic allocator manages the physical page chaining for the linked list
 * of arrays. If the current array page is full, it allocates a new KHO preserved
 * page, links it via next_array_phys, and updates the caller's "current" pointer
 * to point to the newly allocated page.
 *
 * Return: The index of the newly allocated object slot on success, or
 *         a negative PTR_ERR on allocation failure.
 */
