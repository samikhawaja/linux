 ...
 *
 * @refcount:      Recount of the presereved file.
 *
 ...
 *   1. Decremennt the refcount of the file.
 *   2. If refcount is zero, then calls the handler's .unpreserve() callback to
 *      allow the handler to release any resources it allocated.
 *   3. Removes the file from the file_set's internal tracking list.
 *   4. Releases the reference to the 'struct file' that was taken by
 *      luo_preserve_file() via fput(), returning ownership.
 *   5. Frees the memory associated with the internal 'struct luo_file'.
 *   6. Return if file_set is empty, otherwise goto 2.
 *
...
void luo_file_unpreserve_files(struct luo_file_set *file_set)
{
	...
	while (!list_empty(&file_set->files_list)) {
		list_for_each_entry_safe(luo_file, n, &file_set->files_list, list) {
			...
			/*
			 * Refcount of each luo file should be zero to
			 * unpreserve. Some handlers may call
			 * liveupdate_put_file_outgoing to reduce the refcount
			 * to zero if they had called
			 * liveupdate_get_file_outgoing previously.
			 */
