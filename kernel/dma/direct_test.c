						    ...
						    size_t size)
{
	...
	if (dma_is_from_pool(addr1, size)) {
	...
	if (!expected_ret) {
		KUNIT_EXPECT_TRUE_MSG(test, kho_test_pages_preserved(dma_to_phys(&dev, handle1), nr_pages), "Allocated block not tracked in KHO");
		...
		KUNIT_EXPECT_FALSE_MSG(test, kho_test_pages_preserved(dma_to_phys(&dev, handle1), nr_pages), "Allocated block still tracked after free");
	} else {
